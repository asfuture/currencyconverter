...
describe('DolarCanadenseComponent', () => {
  ...
  beforeEach(async () => {
    const cambioServiceMock = jasmine.createSpyObj('CambioService', ['getDolarCanadense']);
  ...
  it('should get cotacao on init', () => {
    ...
    cambioServiceSpy.getDolarCanadense.and.returnValue(of(mockCotacao));
    component.ngOnInit();
    expect(component.cotacaoDolarCanadense).toEqual([mockCotacao]);
  });

  it('should handle error on getCotacao', () => {
    cambioServiceSpy.getDolarCanadense.and.returnValue(throwError('error'));
